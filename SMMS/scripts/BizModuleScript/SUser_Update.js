 ...
 ******************************************************************************/
function doOnload() {
	if (typeof(doOnload_Super) != "undefined") {
		doOnload_Super();
	} // 如果存在页面加载超级方法，则先执行
...
function doChange(obj) {
	if (typeof(doChange_Super) != "undefined") {
		doChange_Super(obj);
	} // 如果存在onChange的超级方法，则先执行
	// //////对于新增操作，页面元素发生改变是要执行的代码在此完成//////////
	if (obj.name == 'LOGINCODE') { // 登录账号
		setFieldValue("ISMODIFYLOGINCODE", "false");
		...
		function validationInfo() { // 验证登录账号信息
			var url = rootPath
					+ "/fmp/SUser/isExistLoginCode?tableModelId=SUser&LOGINCODE="
					+ loginCode;
			comAjax(url, backCallInfo);
			function backCallInfo(msg) {
				if(msg == "exist"){
					showMessage("MSG1236");//此登录账号已存在，请重新输入！
					setFieldValue("LOGINCODE", preLoginCode[0].value);
				}else if(msg == "success"){
					setFieldValue("ISMODIFYLOGINCODE", "true");
					showMessage("MSG1248");//若修改此登录账号，点击保存按钮后将重置该用户的密码为：888888
					return;
				}
			}
		}
	...
	if (obj.name == 'IDCARDNO') { // 身份证号码
		...
		function validationMsg() { // 验证身份证信息
			var url = rootPath
					+ "/fmp/SUser/isExistIdCardNo?tableModelId=SUser&IDCARDNO="
					+ idCardNo;
	...
	if (obj.name == 'FIREDATE') { // 解雇日期
		...
		if (startDate == "" || startDate == null) { // 判断开启日期是否为“空”
			...
			function dateConvDig(str) { // 将日期转换为整型
				var numerical = 0;
				if (str != null && str != "") {
					numerical = parseInt(str.substring(0, 4)
...
function openDeptPop() { // 打开所属部门pop窗口
	...
	if (orgId == "" || orgId == null) { // 判断所属机构是否为“空”
		...
		var freezeCondition = "and (dept.ORGID='" + orgId
				+ "' or dept.ORGID='[ALL]') and dept.STATE='0'";
