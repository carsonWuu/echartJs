   ...
   $.fn.Flowdesign = function(options)
    ...
    $(".process-step").on('click',function(){
        //激活
         ...
         var obj = this;
         alert("window"+$(this).attr("process_id"));
        // defaults.fnClick();
    });
    ...
    jsPlumb.makeTarget(jsPlumb.getSelector(".process-step"), {
        ...
        beforeDrop:function(params){
            if(params.sourceId == params.targetId) 
            	return false;/*不能链接自己*/
            if(params.sourceId=='window001'&&params.targetId=='window999')
            {
                alert("开始不能指向结束");
                return false;
            }
            if(params.sourceId=='window999'&&params.targetId=='window001')
            {
            	alert("结束不能指向开始");
                return false
            }
            if(params.targetId=="window001")
            {
                alert("流程不能指向开始节点");
                return false;
            }
