	...
	if(isNumber(parSkipValue)){			//判断是否为数字
		if(parSkipValue>0 && parSkipValue<=parSum){	//判断是否在页数范围内
			if(parSkipValue != currentPage){  
				var url="<%=rootPath%>/fmp/system/notice/RiskListBiz/MRiskList?tableModelId=SRiskWarnMsg&pq_currentPage="+parSkipValue+"&pq_maxLine=<%=pageSize%>";
...
<body>
 ...
 <div class="home_div_main">
		  ...
		  <div class="home_div_content">
			  ...
			  for (Iterator it = searchDataList.iterator();it.hasNext();){
				...
				%>
				    ...
				    %>
			  		<br>
			        <a href="<%=rootPath%>/fmp/system/notice/RiskDetailBiz/MRiskDetail?tableModelId=SRiskWarnMsg&RID=<%=MsgRID %>">
			        	<%=msgContent %>
			        </a>
			 <% }%>
   ...
   <div align="center">
         <a href="<%=rootPath%>/fmp/system/notice/RiskListBiz/MRiskList?tableModelId=SRiskWarnMsg&pq_currentPage=<%=1 %>&pq_maxLine=<%=pageSize %>" class="link">
                         首页  </a>   &nbsp;                  
          <%         
          //如果当前页数为1，则页数不可以再减
          ...
          %>               
         <a href="<%=rootPath%>/fmp/system/notice/RiskListBiz/MRiskList?tableModelId=SRiskWarnMsg&pq_currentPage=<%=frontPage %>&pq_maxLine=<%=pageSize %>" class="link">
                         上一页  </a>    
       ...
       <%
          ...
          %>               
         <a href="<%=rootPath%>/fmp/system/notice/RiskListBiz/MRiskList?tableModelId=SRiskWarnMsg&pq_currentPage=<%=nextPage%>&pq_maxLine=<%=pageSize %>" >
                           下一页  </a>
                         
           <a href="<%=rootPath%>/fmp/system/notice/RiskListBiz/MRiskList?tableModelId=SRiskWarnMsg&pq_currentPage=<%=sum %>&pq_maxLine=<%=pageSize %>" >
                            尾页  </a>   &nbsp;  
