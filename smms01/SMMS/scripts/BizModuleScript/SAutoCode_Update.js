...
function doChange(obj) {
	...
	if(obj.name == 'CODETYPE'){     //编码类型
		...
		var preOrgId = document.getElementsByName("pre#ORGID")[0].value;
		var preCodeType = document.getElementsByName("pre#CODETYPE")[0].value;
		setErrMessage("ORGID", "");
		setErrMessage("CODETYPE", "");
		if(preOrgId == orgId && preCodeType == codeType){      //判断记录值是否等于修改值
			return;
		}
