...
import com.strutsframe.db.DBConnectionImpl;
...
public class Load_Url_Key_LogFileJob extends BaseJob {
	...
	public void jobExcute(String arg0) throws JobExecutionException {
		...
		if (true) {
			...
			filePath = (new StringBuilder(String.valueOf(FMPContex.getSystemProperty("AC_LOG_KEY_PATH"))))
			...
			backUpPath = (new StringBuilder(String.valueOf(backPath))).append("/key/").append(date).append("/")
			...
			if (iterator1.hasNext()) {
				...
				if (!fileName.startsWith("keyword_url") || !fileName.endsWith(".mdf.ok")) {
					FMPLog.printLog("!fileName.startsWith(\"keyword_url\") || !fileName.endsWith(\".mdf.ok\")");
				} else {
					...
					if (!file.exists()) {
						...
						try {
							...
							String tableRow = "USERNAME,GROUPNAME,IP,SERV_CRC,APP_CRC,LOG_TIME,HOST,TITLE,URL,KEYWORDS,CONTENTPATH";
