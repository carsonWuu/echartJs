...
Calendar.prototype.draw = function() {
	...
	/*
	_cs[_cs.length] = ' <tr>';
	_cs[_cs.length] = '<th colspan="7">';
	_cs[_cs.length] = '<select name=\"hourSelect\" id=\"hourSelect\"><\/select>时&nbsp;&nbsp;&nbsp<select name=\"minitSelect\" id=\"minitSelect\"><\/select>分&nbsp;&nbsp;&nbsp;<select name=\"secendSelect\" id=\"secendSelect\"><\/select>秒';
	_cs[_cs.length] = '<\/th>';
	_cs[_cs.length] = ' <\/tr>';
	*/
	_cs[_cs.length] = ' <tr>';
	...
	this.form.selectTodayButton.onclick = function () {
		...
		if (calendar.dateControl){
			if (typeof(calendar.dateControl.onchange) != "undefined") {
				calendar.dateControl.onchange();
			}	
			calendar.dateControl.date_check(calendar.dateControl);
		}		
		calendar.hide();
	...
	for(var i = 0; i < tds.length; i++) {
		...
		if (dateArray[i]){
			tds[i].onclick = function () {
				...
				calendar.preTrObj = calendar.dateControl.parentNode.parentNode;
				if (calendar.dateControl){
					if (typeof(calendar.dateControl.onchange) != "undefined") {
						calendar.dateControl.onchange();
					}	
...
Calendar.prototype.bindTime = function () {
	...
	var ms = this.form.minitSelect;
	var ss = this.form.secendSelect;
	for (var i = 0; i <= 59; i++){
		ms.options[i] = new Option(i, i);
		ss.options[i] = new Option(i, i);
	}	
};
...
Calendar.prototype.show = function (dateControl, popuControl) {
	...
	if(xy.x + 200 > document.body.offsetWidth){
		// 屏幕右侧高度不足，向左偏移宽度
		this.panel.style.left = (document.body.offsetWidth - 200) + "px";
	} else {
		this.panel.style.left = xy.x + "px";
	}
	

	if($(dateControl).offset().top + 18 + 210 > document.body.offsetHeight){
		// 下方高度不足
		if($(dateControl).offset().top + 18 < 210 || document.body.offsetHeight < 210){
			// 上高度不足210px，或body高度不足210px
			this.panel.style.top = "0px"; // 从顶部显示
			if(typeof(resizeIframe) != "undefined" ){
				resizeIframe(210); // 风控单调整高度，最低210px
			}
		} else {
			//this.panel.style.top = (xy.y + dateControl.offsetHeight - 210) + "px"; // 往上弹出
			this.panel.style.top = $(dateControl).offset().top + 18 -210 +"px";
		}
	} else {
		//this.panel.style.top = (xy.y + dateControl.offsetHeight) + "px";
		this.panel.style.top = $(dateControl).offset().top + 18 +"px";
	}
...
document.writeln('<div id="__calendarPanel" style="position:absolute;visibility:hidden;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:190px;height:1px;top:-100px">');//210
...
__ci.document.writeln('select.month {width:50px;}');
