 ...
 * 给方法装配事件，包括合法性校验
 */
function setObjEvens(objId, objType) {
	var obj = document.getElementById(objId);
	if (!obj){
		return;
	}
	obj.objType = objType;
	//$(obj).attr("objType",objType);
	if (objType.toLowerCase() == "date") {
		obj.readOnly = true;
		obj.onblur = function() {
			//if (!date_check(this)) {
			//	this.value = "";
			//}
			setErrMessage(this.name, "");			
		...
		obj.onclick = function() {
			calendar.show(this);
			/*if (this.value == "" || date_check(this)) {
				calendar.show(this);
			} else {
				this.value = this.DefaultValue();
			}*/
		};
		obj.date_check = function(obj){
			value = obj.value;
			var dateObj;
			var msg = "日期格式错误！";
			if (value == null || value == "") {// 如果未输入任何数据，则不作格式检查和转换。
				return setErrMessage(obj, "");
			}
			...
			if (obj.id.indexOf("detailId_$B$")==0){
				var e_obj=document.getElementById("detailId_$E$"+obj.id.substr(12));
				if ($(e_obj).val()!="" && $(obj).val()!="" &&($(e_obj).val()<$(obj).val())){
					//showMessage("区间查询的开始值必须小于结束值"); 
					setErrMessage(e_obj, "区间查询的开始值不能大于结束值");
					return;
				}
			}else if (obj.id.indexOf("detailId_$E$")==0){
				var b_obj=document.getElementById("detailId_$B$"+obj.id.substr(12));
				if ($(b_obj).val()!="" && $(obj).val()!="" &&($(obj).val()<$(b_obj).val())){
					//showMessage("区间查询的开始值必须小于结束值"); 
					setErrMessage(b_obj, "区间查询的开始值不能大于结束值");
					return;
				}				
			}	
			...
			var value = year + "-" + month + "-" + date;
			obj.value = date_display(value);

			return setErrMessage(obj, "");			
		};
	} else if (objType.toLowerCase() == "datetime") {
		obj.onblur = function() {
			datetime_check(this, /^\d{4}-(0\d|1[0-2])-([0-2]\d|3[01])( ([01]\d|2[0-3])\:[0-5]\d\:[0-5]\d)$/);
		};
		datetime_check(obj, /^\d{4}-(0\d|1[0-2])-([0-2]\d|3[01])( ([01]\d|2[0-3])\:[0-5]\d\:[0-5]\d)$/);
		if (typeof(doBlur) != "undefined") {
			doBlur(this);
		}			
	} else if (objType.toLowerCase() == "time") {
		obj.onblur = function() {
			time_check(this, /([01][0-9]|2[0-3]):[0-5][0-9]/);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		time_check(obj, /([01][0-9]|2[0-3]):[0-5][0-9]/);
	} else if (objType.toLowerCase() == "textarea") {
		obj.onclick = function() {
			if(obj.scrollHeight>30){
				obj.style.posHeight=obj.scrollHeight+5;
			}			
		};
		obj.onclick();
		obj.onblur = function() {
	...
	} else if (objType.toLowerCase() == "money"
		|| objType.toLowerCase() == "money10000"
			|| objType.toLowerCase() == "money100000000") {
		obj.ondblclick = function(){
			showMessage("MSG0090",amountConvChinese(obj.value));//金额大写：%s
		};
		obj.onblur = function() {
			decimal_check(this, 2, true, false, this.min, this.max);
			//obj.title = "金额大写："+amountConvChinese(obj.value);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		...
		obj.onmouseover = function(){
			obj.title = "\n 金额大写："+amountConvChinese(obj.value,objType.toLowerCase());

//			var divObj=document.getElementById("titleDiv");
//			if(divObj){
//			divObj.style.left = window.event.clientX+window.document.body.scrollLeft+"px";
//			divObj.style.top = window.event.clientY+ window.document.body.scrollTop+"px" ;
//			if (divObj.style.display == "none"){
//			divObj.innerHTML="金额大写："+amountConvChinese(obj.value,objType.toLowerCase());
//			divObj.style.display = "";
//			}	
//			}else{
//			var newObj=document.createElement('div');
//			newObj.id = 'titleDiv';
//			newObj.innerHTML="金额大写："+amountConvChinese(obj.value,objType.toLowerCase());
//			newObj.style.width='auto';  
//			newObj.style.border='1px solid #000000';  
//			newObj.style.background='#ffe3a8';  
//			newObj.style.zIndex='99';  
//			newObj.style.position='absolute'; 
//			newObj.style.padding='6px';  
//			newObj.style.left = window.event.clientX+window.document.body.scrollLeft+"px";
//			newObj.style.top = window.event.clientY+ window.document.body.scrollTop+"px" ;
...
//			}
		...
		decimal_check(obj, 2, true, false, obj.min, obj.max);
	} else if (objType.toLowerCase() == "decimal") {
		obj.onblur = function() {
			decimal_check(this, 2, false, false, this.min, this.max);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		decimal_check(obj, 2, false, false, obj.min, obj.max);
	} else if (objType.toLowerCase() == "number") {
		obj.onblur = function() {
			number_check(this, this.min, this.max);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		number_check(obj, obj.min, obj.max);
	} else if (objType.toLowerCase() == "idcard") {
		obj.onblur = function() {
			identity_check(this);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
		...
		identity_check(obj);
	} else if (objType.toLowerCase() == "percent") {
		obj.onblur = function() {
			percent_check(this, 4, true, true, this.max, this.min);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		percent_check(obj, 4, true, true, obj.max, obj.min);
	} else if (objType.toLowerCase() == "permillage") {
		obj.onblur = function() {
			percent_check(this, 5, false, true, this.max, this.min);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		percent_check(obj, 5, false, true, obj.max, obj.min);
	} else if (objType.toLowerCase() == "email") {
		obj.onblur = function() {
			email_check(this, /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		email_check(obj, /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/);
	} else if (objType.toLowerCase() == "mobilephone") {
		obj.onblur = function() {
			var regFormat = /^(13[0-9]{9})|(15[89][0-9]{8})$/;
			var regFormats = /^0*(13|15)\d{9}$/;
			var regFormatss = /^0*(1)\d{10}$/;
			mobilephone_check(this, /^0*(1)\d{10}$/);
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}				
		};
		mobilephone_check(obj, /^0*(1)\d{10}$/);
	} else if (objType.toLowerCase() == "term") {
		var dispObj = document.getElementById("disp_"+objId);
		dispObj.objType=objType;
		dispObj.trueObj=obj;
		dispObj.selectObj=document.getElementById("termselect_"+obj.name);
		//$(dispObj).attr("objType",objType);
		//$(dispObj).attr("trueObj",obj);
		//$(dispObj).attr("selectObj",document.getElementById("termselect_"+obj.name));
//		dispObj.trueObj = obj;
//		dispObj.selectObj = document.getElementById("termselect_"+obj.name);

		if (dispObj.selectObj){
			var sele_obj=dispObj.selectObj;
			sele_obj.dispObj=dispObj;
			//$(dispObj).attr("selectObj").attr("dispObj",dispObj);
			sele_obj.onchange = function(){
				dispObj.onblur();
			};
		}

		dispObj.onblur = function() {
			var trueValue = 0;
			var oldValue = obj.value;
			if (obj){
				if (number_check(dispObj.trueObj, this.min, this.max)){
					if (dispObj.selectObj && dispObj.selectObj.value == '1'){//如果选择的是月
						trueValue = moneyToFloat(dispObj.value)*1000;
						obj.value = trueValue;
					}else if (dispObj.selectObj && dispObj.selectObj.value == '2'){  //如果选择日
						trueValue = moneyToFloat(dispObj.value);
						obj.value = trueValue;
					}				 
		...
		obj.dispObj = dispObj;
		number_check(obj, obj.min, obj.max);
	} else if (objType.toLowerCase() == "pop") {
		var dispObj = document.getElementById("disp_"+objId);
		if(!dispObj){
			return;
		}
		//$(dispObj).attr("objType",objType);  //$(obj).attr("objType")
		dispObj.objType = objType;
		obj.onblur = function() {
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
		...
		//$(obj).attr("dispObj",dispObj);
		obj.dispObj = dispObj;
	} else if (objType.toLowerCase() == "select") {
		obj.onblur = function() {
			if (typeof(doBlur) != "undefined") {
				doBlur(this);
			}	
	...
	obj.subCheck = function() {
		var requiredvalue = 1;
		var msg = "必填项不允许为空！";
		if ($(this).attr("fieldRequired")) {
			if (this.value == null || trim(this.value) == "") {
				requiredvalue = 0;
				setErrMessage(this.name, msg);
				return requiredvalue;
			}
		}
		if (document.getElementById('errInfo_' + this.name)){
			var errMsg = document.getElementById('errInfo_' + this.name).innerText;   
			if (trim(errMsg) != "") {
				requiredvalue = 0;
			}
		}
	...
	obj.onchange = function() {
		if (typeof(addChangeRids) != "undefined" && addChangeRids  ) {
			addChangeRids(this);
		}
		if (typeof(doChange) != "undefined" && doChange  ) {
			doChange(obj);
		}	
	};
 ...
 * showSymbol  是否显示货币符号
 */
function decimal_display(value, scale, showComma, showSymbol) {
 ...
 * @param obj			要检查的对象
 * @param scale			小数位
 * @param showComma		是否需要3位一逗
 * @param showSymbol	是否需要显示金额符号
 * @param min			最小值范围
 * @param max			最大值范围
 * @return
 */
function decimal_check(obj, scale, showComma, showSymbol, min, max) {
	var msg = "请输入数值型数据";
	var value = obj.value;
	if (value == null || value == "") {
		return setErrMessage(obj, "");
	...
	var valueD_copy = valueD;
	if(isNegative){
		valueD_copy = -valueD_copy;
	}
	if ((min != "" && valueD_copy < min) || (max != "" && valueD_copy > max)) {// 校验最大值和最小值
		return setErrMessage(obj, "数据超出范围[" + min + "~" + max + "]");
	}
	...
	obj.value = decimal_display(value, scale, showComma, showSymbol);
	return setErrMessage(obj, "");
...
function identity_check(obj) {
	...
	if ((parseInt(day, 10) != parseInt(dateObj.getDate()))
			|| (parseInt(month, 10) != parseInt((dateObj.getMonth() + 1)))) {
		return setErrMessage(obj, msg);
	}
	if (length == 18) {
		var wi = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];
		var verifyCodeList = ["1", "0", "X", "9", "8", "7", "6", "5", "4", "3",
		                      "2"];
		var totalCode = 0;
		for (var k = 0; k < 17; k++) {
...
/** **********************整数类型的输入域***************************** */
function number_display(obj, min, max) {
	var msg = "请输入正确的整数";
	var value = obj.value;
	if (value == null || value == "") {
		return "";// 如果未输入任何数据，则不作格式转换
...
function number_check(obj, min, max) {
	var value = obj.value;
	var msg = "请输入数值字符";
	if (value == null || value == "") {
		return setErrMessage(obj, "");
	...
	if ((min != "" && valueD < min) || (max != "" && valueD > max)) {// 校验最小值、最小值
		return setErrMessage(obj, msg);
	}
	obj.value = number_display(obj, min, max);
	return setErrMessage(obj, "");
}

/** **********************百分比类型的输入域***************************** */

function percent_display(value, scale, isPercent, showComma) {
	if (value == null || value == "") {// 无真实值或真实值为空串，则直接返回空串
		return "";
	}

	var valueD = value*1;//parseFloat(value);
	...
	valueD = Math.round(valueD * 1000000)/1000000; 
	value = valueD.toString();
	var d = value.indexOf(".");
	var integer = value;
	var decimal = "";
	...
	if (scale > 0) {
		if (decimal.length < scale){
			scaleStr = decimal;
			while (scaleStr.length < scale) {
				scaleStr += "0";
			}
		}else{
			//for (var i=0;i<scale;i++){
			//	scaleStr += decimal.substring(i, i+1);
			//}		
			scaleStr = decimal;
		}

		value = integer + "." + scaleStr;
	} else {
		value = integer;
	}
 ...
 * @param obj			需要检查的对象
 * @param scale			小数位
 * @param isPercent		是否百分比，否则千分比
 * @param showComma		是否采用3位一逗显示
 * @param max			最大值范围	
 * @param min			最小值范围
 * @return
 */
function percent_check(obj, scale, isPercent, showComma, max, min) {
	var value = obj.value;
	var msg = "请输入正确的千分比";
	if (isPercent) {
		msg = "请输入正确的百分比";
	...
	if (value == null || value == "") {
		return setErrMessage(obj, "");// 如果未输入任何数据，则不作格式检查和转换。
	}
	...
	//valueD = Math.round(valueD);
	//valueD /= Math.pow(10, scale);
	if ((min != "" && valueD < min) || (max != "" && valueD > max)) {// 校验最大值和最小值
		return setErrMessage(obj, "数据超出范围[" + min + "~" + max + "]");
	}
	...
	obj.value = percent_display(value, scale, isPercent, showComma);
	return setErrMessage(obj, "");
}
...
function time_check(obj, regFormat) {
	var value = obj.value;
	var msg = "时间格式错误，正确格式为 hh:mm 如 11:45";
	if (value == null || value == "") {
		return setErrMessage(obj, "");
	}
...
function datetime_check(obj, regFormat) {
	var value = obj.value;
	var msg = "时间格式错误，正确格式如 2011-10-07 12:11:45";
	if (value == null || value == "") {
		return setErrMessage(obj, "");
	}

	var reg = new RegExp(regFormat);
	var checkres = reg.test(trimRight(value)); //去除右边空格后匹对
	if (!checkres) {
		return setErrMessage(obj, msg);
	} 
...
function mobilephone_check(obj, regFormat) {
	var value = obj.value;
	var msg = "手机号码格式错误";
	if (value == null || value == "") {
		return setErrMessage(obj, "");
...
function email_check(obj, regFormat) {
	var value = obj.value;
	var msg = "邮件格式错误";
	if (value == null || value == "") {
		return setErrMessage(obj, "");
...
function onCheckBoxClick(obj){
	var fldName = obj.name.substr(4);   //去掉前面的  cbx_
	...
	setFieldValue(fldName,fldValue);
}

function onRadioClick(obj){
	var fldName = obj.name.substr(4);   //去掉前面的  rdo_
	setFieldValue(fldName, obj.value);
}
	...
	if (form && form.elements){
		...
		for (var i = 0; i < form.elements.length; i++) {
			...
			if (obj != null && obj[0] != null) {
				var divObj = $('#div_'+form[i].name);
				if (divObj[0]){
					if (divObj[0].style.display != "none"){//如果字段不是隐藏状态
						if (obj[0].subCheck != null) {
							requiredvalue = obj[0].subCheck();
							result = result && requiredvalue;
						}
					}
				}else{  //如果不存在外层DIV,例如如在列表中输入
					if (obj[0].subCheck != null) {
						if (obj.is(':visible')){//如果字段不是隐藏状态
							requiredvalue = obj[0].subCheck();
							result = result && requiredvalue;
						}
		...
		for (var i = 0; i < spans.length; i++) {
			if (spans[i].id.substr(0,8) == "errInfo_"){
				if (trim(spans[i].innerText) != ""){
					//showMessage();
					return false;
				}
			}
 ...
 * @return
 */
function setErrMessage(field, msg) {
	if (typeof field != 'object') {
		field = getFieldObject(field);
		if(field == null){//此处判断取到的对象为null时返回不做以下操作
			return ;
		}
	}
	if (msg !=null && msg != ""){
	...
	var errObj = $("#"+$(field).attr("errInfoId"));
	if (errObj) {
		errObj.html(msg);
	}	
...
function fillCity(thisObj) {
	try{
		aCity = areaData.split(",");
		var j = 1;
		for (i = 1; i <= aCity.length; i++) {
			if (aCity[i - 1].substring(2, 6) == "0000") {
				document.getElementById(thisObj.name + "_province").options[j] = new Option(
...
function setCity(thisObj) {
	try{
		provinceCode = thisObj.value;
		var j = 1;
		...
		for (i = 1; i <= aCity.length; i++) {
			if (aCity[i - 1].substring(0, 2) == provinceCode
				...
				document.getElementById(thisObj.name + "_city").options[j] = new Option(
						aCity[i - 1].substring(7, aCity[i - 1].length),
						aCity[i - 1].substring(0, 6));
				j++;
			}
		}
		document.getElementById(thisObj.name + "_city").length = j;
		setCounty(document.getElementById(thisObj.name + "_city"), '000000');
	}catch(e){
		//吃掉异常，保证JS能继续走下去
	}	
}

function setCounty(thisObj, cityCode) {
	try{
		var j = 1;
		...
		for (i = 1; i <= aCity.length; i++) {
			if (aCity[i - 1].substring(0, 4) == cityCode
					&& aCity[i - 1].substring(4, 6) != "00") {
				document.getElementById(thisObj.name + "_county").options[j] = new Option(
						aCity[i - 1].substring(7, aCity[i - 1].length),
						aCity[i - 1].substring(0, 6));
				j++;
			}
		...
		if (thisObj) {
			document.getElementById(thisObj.name + "_county").length = j;
		}
	}catch(e){
		//吃掉异常，保证JS能继续走下去
	}	
...
function getArea(thisObj) {
	try{
		document.getElementById("detailId_" + thisObj.name).value = "";
		if (document.getElementById(thisObj.name + "_province").value != "") {
			document.getElementById("detailId_" + thisObj.name).value = document
			.getElementById(thisObj.name + "_province").value;
		}
		if (document.getElementById(thisObj.name + "_city").value != "") {
			document.getElementById("detailId_" + thisObj.name).value = document
			.getElementById(thisObj.name + "_city").value;
		}
		if (document.getElementById(thisObj.name + "_county").value != "") {
			document.getElementById("detailId_" + thisObj.name).value = document
			.getElementById(thisObj.name + "_county").value;
		}
	}catch(e){
		//吃掉异常，保证JS能继续走下去
	}
...
function setArea(thisObj) {
	try{
		for (i = 1; i <= document.getElementById(thisObj.name + "_province").length; i++) {
			if (document.getElementById(thisObj.name + "_province").options[i - 1].value
					.substring(0, 2) == document.getElementById("detailId_"
							+ thisObj.name).value.substring(0, 2)) {
				document.getElementById(thisObj.name + "_province").selectedIndex = i- 1;
			}
		}
		setCity(document.getElementById(thisObj.name + "_province"));
		for (i = 1; i <= document.getElementById(thisObj.name + "_city").length; i++) {
			if (document.getElementById(thisObj.name + "_city").options[i - 1].value
					.substring(0, 4) == document.getElementById("detailId_"
							+ thisObj.name).value.substring(0, 4)) {
				document.getElementById(thisObj.name + "_city").selectedIndex = i- 1;
			}
		}
		setCounty(document.getElementById(thisObj.name + "_city"), document
				.getElementById(thisObj.name + "_city").value);
		for (i = 1; i <= document.getElementById(thisObj.name + "_county").length; i++) {
			if (document.getElementById(thisObj.name + "_county").options[i - 1].value == document
					.getElementById("detailId_" + thisObj.name).value) {
				document.getElementById(thisObj.name + "_county").selectedIndex = i- 1;
			}
		}
	}catch(e){
		//吃掉异常，保证JS能继续走下去
...
function setMaxLength(field, maxlimit) {
	var len = field.value.replace(/[^\x00-\xff]/g, "**").length;
	var tempString = field.value;
	var tmpstr = "";
	if (len > maxlimit) {
		for (var i = 0; i < maxlimit; i++) {
			if (tmpstr.replace(/[^\x00-\xff]/g, "**").length < maxlimit)
				tmpstr = tempString.substr(0, i + 1);
			else
				break;
		}
		if (tmpstr.replace(/[^\x00-\xff]/g, "**").length > maxlimit)
 ...
 * @return
 */
function setFieldDisplay(fldName,yn){
	var obj = document.getElementById('div_'+fldName);
	if (obj){
		if (yn){
 ...
 * @return
 */
function setFieldRequired(fldName,yn){
	var obj = document.getElementById("detailId_"+fldName);
	if (typeof(obj.fieldRequired) != "undefined") {
		obj.fieldRequired = yn;
	}
}
 ...
 * 控制单选框和复选框的选项显示和隐藏
 * @param fldName    字段名
 * @param valuestr   逗号分隔的值
 * @param disp   隐藏="none"  显示=""
 * @return
 */
function setBoxDisplay(fldName,valuestr,disp){
	var values = valuestr.split(",");
	for (var i = 0; i < values.length; i++){
		var obj = document.getElementById("boxitemspan_"+fldName+values[i]);
		if (obj){
 ...
 * @param fldName  字段名
 * @param value    字段真实值
 * @param dispValue  字段显示值
 * @return
 */
function setFieldValue(fldName,value,dispValue){
	if (dispValue == null){
		dispValue = value;
	}	 		
	var schobj = document.getElementById("search_hiddenfld_id_"+fldName);
	if (schobj){
		schobj.value = value;	 
	}
	var disobj = document.getElementById("disp_detailId_"+fldName);
	if (disobj){
		disobj.value = dispValue;
		disobj.truevalue = value;
	}	
	var obj = document.getElementById("detailId_"+fldName);
	if (obj){
	...
	var aobj = document.getElementById("a_"+fldName);
	if (aobj){
		aobj.innerText = dispValue; 
		aobj.value = dispValue; 
		aobj.truevalue = value; 
	}		 
 ...
 * @return
 */
function getFieldObject(fldName){
	var obj = document.getElementById("detailId_"+fldName);
	if (obj == null){
		obj = document.getElementById("search_hiddenfld_id_"+fldName);
	}
	return obj;
}
 ...
 * @return
 */
function reSetFieldValue(fldName){
	var obj = getFieldObject(fldName);
 ...
 * 设置列表中的数据值
 * @param rid   		列表记录的RID
 * @param fldName  		要设置的字段名
 * @param value   		要设置的值
 * @param dispValue		要设置的显示值
 * @return
 */
function setDataListValue(rid,fldName,value,dispValue){
	setFieldValue(fldName+"_rid_"+rid, value,dispValue);
	var obj = document.getElementById("a_"+fldName+"_rid_"+rid);
	if (obj){
		if(dispValue!="" && dispValue!=null){
			obj.innerText = dispValue;
		}else{
			obj.innerText = value;
		}
	}
 ...
 * @return
 */
function setDataListFieldReadOnly(rid,fldName,yn){
	setReadOnly("detailId_"+fldName+"_rid_"+rid, yn);
}
 ...
 * @return
 */
function setDataListFieldDisplay(rid,fldName,yn){
	var obj = document.getElementById('detailId_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
			obj.fieldRequired = true;
		}else{
			obj.style.display = "none";
			obj.fieldRequired = false;
		}
	}

	obj = document.getElementById('disp_detailId_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById("a_"+fldName+"_rid_"+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById(fldName+"_rid_"+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById('termselect_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById('termselect_span1_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById('termselect_span2_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById('div_fix_select1_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
	...
	obj = document.getElementById('div_fix_select2_'+fldName+'_rid_'+rid);
	if (obj){
		if (yn){
			obj.style.display = "";
		}else{
 ...
 * @return
 */
function getDataListTrueValue(rid,fldName){
	var obj = document.getElementById(fldName+"_rid_"+rid);
	if (obj){
		return obj.value;
	}
	
	obj = document.getElementById("detailId_"+fldName+"_rid_"+rid);
	if (obj){
		return obj.value;
	}
	return getDataListDisplayValue(rid,fldName);
}
 ...
 * @return
 */
function getDataListDisplayValue(rid,fldName){
	var obj = document.getElementById("a_"+fldName+"_rid_"+rid);
	if(obj){
		var dispValue=document.getElementById("a_"+fldName+"_rid_"+rid).innerText;
	}else{
		obj = document.getElementById("detailId_"+fldName+"_rid_"+rid);
		if(obj){
			if(typeof(obj.options)!="undefined"){
				...
				var dis_obj = document.getElementById("disp_detailId_"+fldName+"_rid_"+rid);
				if (dis_obj){
					return dis_obj.value;
				}
				
				dis_obj = document.getElementById(fldName+"_rid_"+rid);
				if (dis_obj){
					return dis_obj.value;
				}
				
				dis_obj = document.getElementById("detailId_"+fldName+"_rid_"+rid);
				if (dis_obj){
					return dis_obj.value;
				}
			}
 ...
 * @return
 */
function getFieldTrueValue(fldName){
	var trueValue;
	var trueValueObj = document.getElementById("detailId_"+fldName);
	if (trueValueObj){
		trueValue = trueValueObj.value;
		if(trueValueObj.objType){
			if(trueValueObj.objType.toLowerCase() == "money"
				|| trueValueObj.objType.toLowerCase() == "money10000"
					|| trueValueObj.objType.toLowerCase() == "money100000000"){
				trueValue = trueValue.replace(/\,/g, "" );
			}
			else if(trueValueObj.objType.toLowerCase() == "percent"){
				trueValue = trueValue.replace(/\%/g, "" );
				trueValue = trueValue.replace(/\,/g, "" );
				trueValue = trueValue /100; 
			}else if(trueValueObj.objType.toLowerCase() == "date"){
				if (trueValueObj.dataType == "INT"){
					trueValue = dateToInt(trueValue);
				}
			}
		}
	}
 ...
 * @return
 */
function getFieldDisplayValue(fldName){
	var obj = document.getElementById("a_"+fldName);
	var dispValue=null;
	if(obj){
		dispValue=document.getElementById("a_"+fldName).innerText;
	}else{	
		var disobj = document.getElementById("disp_detailId_"+fldName);
		if (disobj){
			dispValue = disobj.value ; 
		}else{

			var trueValueObj = document.getElementById("detailId_"+fldName);
			if(trueValueObj){
				if($(trueValueObj).attr("objType") == 'select'){
					dispValue = trueValueObj.options[trueValueObj.options.selectedIndex].text;
				}
			}

			if(dispValue==null){

				dispValue = getFieldTrueValue(fldName);
			}
		}
	}
	return dispValue;
 ...
 * @return
 */
function getFieldOldValue(fldName){
	var oldValueObj = document.getElementById("pre#"+fldName);
	var oldValue;
	if (oldValueObj){
...
/**
 * 设置下拉框待选项的显示或者隐藏
 * 例子：selectOptionsDisplay('INDIV_NTN','03,04,05',false);
 * @param fldName  下拉框对应字段名称
 * @param options	逗号分隔要显示或者隐藏的选项键
 * @param isShow	显示或者隐藏  true = 显示   false=隐藏
 * @return
 */
function selectOptionsDisplay(fldName, options, isShow) {
	var selObj = getFieldObject(fldName);
	if (selObj){
		if (!selObj.hiddenOptions) {
			selObj.hiddenOptions = new Array(); 
		}
		options = ','+options+',';
		if (isShow){
			for ( var i = 0; i < selObj.hiddenOptions.length; i++) {
				if (options.indexOf(','+selObj.hiddenOptions[i].value+',') != -1){
					selObj.options.add(selObj.hiddenOptions[i]);
					selObj.hiddenOptions.splice(i,1); 
					if (i>-1){
						i--;
					}						
				}
			...
			for ( var i = 0; i < selObj.options.length; i++) {
				if (options.indexOf(','+selObj.options[i].value+',') != -1){
					selObj.hiddenOptions[selObj.hiddenOptions.length] = selObj.options[i];
					selObj.remove(i);
					if (i>-1){
						i--;
					}
 ...
 * 设置对象是否只读
 * @param ObjId   对象ID
 * @param yn      是否只读(true/false)
 * @return
 */
function setReadOnly(ObjId,yn){
	var obj = document.getElementById(ObjId);
	if (obj){
		obj.disabled = yn;
		obj.readOnly = yn;
 ...
 * @return
 */
function setFieldReadOnly(fldName,yn){
	var obj = document.getElementById("disp_detailId_"+fldName);
	if (obj == null){
		obj = document.getElementById("detailId_"+fldName);
	}
	if ($(obj).attr("objType") == "area"){
		obj = document.getElementById(fldName+"_province");
		if (obj){
			obj.disabled = yn;
			obj.readOnly = yn;
		}	
		obj = document.getElementById(fldName+"_city");
		if (obj){
			obj.disabled = yn;
			obj.readOnly = yn;
		}	
		obj = document.getElementById(fldName+"_county");
		if (obj){
			obj.disabled = yn;
			obj.readOnly = yn;
		}			
	}else if ($(obj).attr("objType") == "checkbox"){
		var checkBoxs = document.getElementsByName("cbx_"+obj.name);
		for(var i=0;i<checkBoxs.length;i++){
			checkBoxs[i].disabled = yn;
		}
	}else if ($(obj).attr("objType") == "radio"){
		var radioBoxs = document.getElementsByName("rdo_"+obj.name);
		for(var i=0;i<radioBoxs.length;i++){
			radioBoxs[i].disabled = yn;
		}
	}else if ($(obj).attr("objType") == "term"){
		if (obj){
			obj.disabled = yn;
			obj.readOnly = yn;
		}		

		obj = document.getElementById("termselect_"+$(obj).attr("trueObj").name);
		if (obj){
			obj.disabled = yn;
			obj.readOnly = yn;
		}
	}else if ($(obj).attr("objType") == "pop"){
		if (obj){
			if (yn){
				obj.oldClassName = obj.className;
				obj.className = "d_field_readonly_text_input";
			}else{
				if (typeof obj.oldClassName !="undefined"){
					obj.className = obj.oldClassName;
				}
			}
		}
		obj = document.getElementById("btn_"+obj.name);
		if (obj){
			if (yn){
				obj.style.display="none";//pop按钮隐藏
			}else{
				obj.style.display="";    //显示pop按钮
			}
		}
	}else{
		if (obj){
...
/**
 * 将金额转换中文大写
 * @param  numberValue	 金额值
 * @return chineseValue  中文大写
 */
function amountConvChinese(numberValue,moneyUnit){
	var numberValue = parseFloat(numberValue.replace(/,/g,""));   //将money转换成Float
	var chineseValue="";                                          //转换后的汉字金额   
	var transValue = "";											// 转为万元单位后金额
	if('money' == moneyUnit){							//对于非万元单位的金额显示万元
		if(isNaN(numberValue)){
			transValue = "\n\n 合  0.00万元";
		}else{
			transValue = "\n\n 合 "+String(FloatDiv(numberValue,parseFloat(10000),6)) + "万元";
		}
	}
	numberValue = new String(Math.round(numberValue*100));        //数字金额
	var digitalStr = "零壹贰叁肆伍陆柒捌玖";                       //汉字数字字符串
	var unitStr = "万仟佰拾亿仟佰拾万仟佰拾元角分";                 //对应单位字符串
	var digitalCh;                                                //数字的汉语读法   
	var unitCh;                                                   //数字位的汉字读法   
	var contZeroNum = 0;                                          //连续零值的个数   
	var specValue;                                                //指定位置的数值   
	var len = numberValue.length;                                 //所需转换的数字金额字符串的长度   

	if(len > 15){                                                 //判断数字金额字符串的长度是否大于15
		var errMessage = "数字金额过大，已超出计算范围，请重新输入";   
		return errMessage;
	}   
	if(numberValue == 0){                                         //判断数字金额是否为0
		chineseValue = "零元整";
		return chineseValue;   
	} 

	unitStr = unitStr.substr(unitStr.length - len, len);          //获取数字金额位数的对应单位字符串
	for(var i = 0; i < len; i++){
		specValue = parseInt(numberValue.charAt(i),10);           //获取某位需转换的特定值（10代表进制）
		if(i != (len - 3) && i != (len - 7) && i != (len - 11) && i !=(len - 15)){  //判断该位是否是万亿，亿，万，元位等关键位   
			if(specValue == 0){                                   //判断特定值是否为“0”
				digitalCh = "";   
				unitCh = "";   
				contZeroNum = contZeroNum + 1;  
			}else {
				if(contZeroNum != 0){                             //判断连续零值的个数是否不为“0”
					digitalCh = "零" + digitalStr.charAt(specValue);
					unitCh = unitStr.charAt(i);   
					contZeroNum = 0;
				}else{
					digitalCh = digitalStr.charAt(specValue);   
					unitCh = unitStr.charAt(i);   
					contZeroNum = 0;
				}
			}		 
		}else{
			if(specValue != 0){                                  //判断特定值是否不为“0”
				if(contZeroNum != 0){                            //判断连续零值的个数是否不为“0”
					digitalCh = "零" + digitalStr.charAt(specValue);   
					unitCh = unitStr.charAt(i);  
					contZeroNum = 0; 
				...
				if(contZeroNum >= 3){                            //判断连续零值的个数是否大于或等于“3”
					digitalCh = "";   
					unitCh = "";   
					contZeroNum = contZeroNum + 1;   
				}else{
			...
			if(i == (len - 11) || i == (len - 3)){               //判断该位是否是亿位或元位   
				unitCh = unitStr.charAt(i);   
			}   
		}   
		chineseValue = chineseValue + digitalCh + unitCh;   
	}

	//if(specValue == 0){                                          //最后一位（分）为0时，加上“整”   
	//	chineseValue = chineseValue + "整";   
	//}

	return chineseValue+transValue;
}
...
function getBirthDayByIdcard(idcard){
	...
	if(idcard.length == 15){                           //判断身份证号码的位数是否等于“15”
		year = "19" + idcard.substring(6,8);
		month = idcard.substring(8,10);
		day = idcard.substring(10,12);
	}else if(idcard.length == 18){                     //判断身份证号码的位数是否等于“18”
...
function getSexByIdCard(idcard){
	...
	if(parseInt(idcard.charAt(tagLocation)/2)*2 != idcard.charAt(tagLocation)){		//判断位数(15位和17位)的数值是否为奇数
		sex = "1";			//'1'代表男性
	}else{
		sex = "2";			//'2'代表女性
	}
...
/**
 * 关于证件类型、证件号码、出生日期的相互关系
 * @param changeingFld   修改中的字段名
 * @param certType  证件类型
 * @param certCode  证件号码
 * @param birthday  出生日期
 * @return
 */
function onCertInfoChange(changeingFld,certType,certCode,birthday,checkExists){
	if(changeingFld.indexOf("_rid_") !=-1){//若是在列表上面的修改的数值，则其id需要做如下处理
		certType += changeingFld.substring(changeingFld.indexOf("_"));
		certCode += changeingFld.substring(changeingFld.indexOf("_"));
		birthday += changeingFld.substring(changeingFld.indexOf("_"));
	}
	var certTypeStr,certCodeStr,birthdayStr;
	if(certType != null && certType != ""){        //判断传来的证件类型参数是否为”空“
		if(document.getElementById("detailId_"+certType) != null){
			certTypeStr = document.getElementById("detailId_"+certType).value;
		}
	}else{
		certTypeStr = "tag";
		setObjEvens("detailId_" + certCode,"idcard");
	}
	if(certCode != null && certCode != ""){        //判断传来的证件号码参数是否为”空“
		if(document.getElementById("detailId_"+certCode) != null){
			certCodeStr = document.getElementById("detailId_"+certCode).value;
		}
	}else{
		certCodeStr = "tag";
	}
	if(birthday != null && birthday != ""){        //判断传来的出生日期参数是否为”空“
		if(document.getElementById("detailId_"+birthday) != null){
			birthdayStr = document.getElementById("detailId_"+birthday).value;
		}
	}else{
		birthdayStr = "tag";
	}
	if(changeingFld == certCode){
		if(certCodeStr != null && certCodeStr != ""){          //判断证件号码是否不为“空”
			if (certTypeStr == ""){                            //判断证件类型是否为“空”
				showMessage("MSG0038");//请先选择证件类型！
				setFieldValue(certCode,"");
			}else if(certTypeStr == "10" || certTypeStr == "tag"){             //判断证件类型是否为身份证
				setObjEvens("detailId_" + certCode,"idcard");
				setFieldValue(birthday,getBirthDayByIdcard(certCodeStr));
			}		
		}	
		if (checkExists == true){
			checkCertCodeMsg(certCode);
		}
	}

	if(changeingFld == certType){	                                   
		if ( certTypeStr == "10" || certTypeStr == "tag"){	  //判断证件类型是否为身份证  
			setObjEvens("detailId_" + certCode,"idcard");
		}else{
			setObjEvens("detailId_" + certCode,"");
			setErrMessage(certCode, "");
		}
	}

	if(changeingFld == birthday){
		if(certTypeStr == "10" || certTypeStr == "tag"){              //判断证件类型是否为身份证
			if(certCodeStr == null || certCodeStr == ""){             //判断证件号码是否为“空”
				if(birthdayStr != null && birthdayStr != ""){         //判断出生日期是否不为“空”
					setFieldValue(birthday,"");
					showMessage("MSG0007");//请先填写证件号码，再根据证件号码获取出生日期！
				}
			}			
		}
	}	
 ...
 * @return
 */
function checkCertCodeMsg(certCode){
	if(document.getElementById("detailId_"+certCode) == null){
		return;
	}
	if (document.getElementById("detailId_"+certCode).value == document.getElementById("detailId_"+certCode).defaultValue){
		return;
	}
	var certCodeStr = document.getElementById("detailId_"+certCode).value;
//	var preCertCode = document.getElementsByName("pre#"+certCode);
//	if(preCertCode[0] != null && preCertCode[0] != "undefined"){        //判断该对象是否存在
//	if(preCertCode[0].value != certCodeStr){                           //判断记录值是否不等于修改值
//	validationMsg();
//	}
//	}else{
//	validationMsg();
//	}
	...
	function validationMsg(){ 
		var url=rootPath+"/creadit/CrdValidateBiz/isExistCertCode?tableModelId=CrdCustBase_Indi&CERTCODE="+certCodeStr; 
		...
		function backCall(msg) {
			if (msg == "Success") {
				...
				setFieldValue(certCode, "", "");
				if ("oldExist" == msg) {
					setErrMessage(certCode, "此证件号码("+certCodeStr+")已被删除，请通知管理员恢复数据！");
				} else if ("newExist" == msg) {
					setErrMessage(certCode, "此证件号码("+certCodeStr+")已存在，请重新输入！");
				} else {
					setErrMessage(certCode, "未知的错误！" + msg);
				}
			}
		}	
	}
...
/**
 * 根据流水号字段值和产品编号字段名获取该项目下的产品编号下拉框
 * @param caseCodeValue       流水号字段值
 * @param productCodeFld      产品编号字段名
 * @return
 */
function getProductSelect(caseCodeValue, productCodeFld){
	if(document.getElementById("detailId_"+productCodeFld) == null || document.getElementById("detailId_"+productCodeFld) == undefined){
		return;
	}
	var url = rootPath + "/credit/caseManage/caseFeeInfo/CrdCaseFeePaymentBiz/getProductInfo?CASECODE="+caseCodeValue;		
	XMLHttp.urlSubmit(comUrl(url),backCall_getMessageData);	
	function backCall_getMessageData(productDataStr){
		document.getElementById("detailId_"+productCodeFld).options.length = 0;	
		document.getElementById("detailId_"+productCodeFld).options.add(new Option("-- 请选择 --",""));

		var productCode;
		var productName;
		var arr = productDataStr.split('#;#');
		var brr=new Array();
		for (var i=0; i<arr.length-1; i++){
			brr[i] = arr[i].split('#=#');
			productCode = brr[i][0];
			productName = brr[i][1];
			if(productCode != "" && productName != null){
				document.getElementById("detailId_"+productCodeFld).options.add(new Option(productName,productCode)); 
			}
		}

		var pdc = getFieldObject(productCodeFld).oldvalue; 
		var selectObj = document.getElementById("detailId_"+productCodeFld);
		for(var i=0;i<selectObj.options.length;i++){
			if(selectObj.options[i].value==pdc){        //判断下拉框选项值
...
function moneyToFloat(moneyValue){
	...
	if (moneyValue==null  || moneyValue=="" || moneyValue=="undefined"){
		return 0;
	}
 ...
 * @return
 */
function floatToMoney(floatValue){
	if (floatValue==null || floatValue=="" || floatValue=="undefined"){
		return "0.00";
	}	
	return decimal_display(floatValue,2,true,false);
}
...
function percentToFloat(percentValue){
	if (percentValue==null  || percentValue=="" || percentValue=="undefined"){
		return "0";
	}
	percentValue = percentValue.replace(/,/g,"");
	percentValue = percentValue.replace(/￥/g,"");
	//数值的最后一位是否是百分号，如果是则为true，否则为false
	var isPercent = (percentValue.lastIndexOf("%") == percentValue.length-1)?true:false;
	percentValue = percentValue.replace(/%/g,"");
	if(isPercent){		//判断是否是百分比格式，否则提示格式错误
 ...
 * @return
 */
function floatToPercent(floatValue){
	if (floatValue==null || floatValue=="" || floatValue=="undefined"){
		return "0.00%";
	}	
	return percent_display(floatValue, 2, true, true);
}

/**
 * 获取打印查看页面的字段转换后的id
 * @param tblMldId	表模型ID
 * @param fldName	字段名
 * @param rid		RID
 * @return
 */
function toPrintViewId(tblMldId,rid,fldName){
	return tblMldId+"_"+fldName+"_rid_"+rid;
}


/**
 * 比较两日期的先后，起始日期应该小于到期日期
 * @param currFld 当前字段id
 * @param startDateFld  开始日期的字段ID
 * @param endDateFld	结束日期的字段ID
 * @return
 */
function compareFldDate(currFld,startDateFld,endDateFld){
	if(currFld == startDateFld || currFld == endDateFld){
		var startDate = getFieldTrueValue(startDateFld);
		var endDate = getFieldTrueValue(endDateFld);
		if(startDate !="" &&  endDate != ""){
			if(startDate > endDate){
				setErrMessage(currFld, "起始日期应该小于结束日期");
				reSetFieldValue(currFld);
			}
		}
	}
}
...
/**
 * 日期加上期限返回日期
 * @param dates	日期型的字符串  如  '2013-01-11'
 * @param term  期限  3 = 3天    3000 = 3个月
 * @return  返回日期型字符串
 */
function dateAddTerm(dates,term){
	term = term * 1;
	//date= new Date();
	//date.setFullYear(dates.substr(0,4),dates.substr(5,2)-1, dates.substr(8,2));
	var date = new Date(Date.parse(dates.replace(/-/,   "/"))); 
	if (term > 999 || term < -999){
		date.setMonth(date.getMonth()+term/1000); 
	}else{
	...
	var currDay = (day < 10) ? ('0' + day) : day;
	var newDate = year + "-" + currMonth + "-" + currDay;        //得到yyyy-mm-dd格式的当前日期
	return newDate;
}
 ...
 * @return
 */
function dateToInt(dates){
	var date = new Date(Date.parse(dates.replace(/-/,   "/"))); 
	var dateBase = new Date(Date.parse("1899/12/31"));
	return(date - dateBase)/86400000;
}
